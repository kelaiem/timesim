...
export default {
  ...
  ['Honesty note, both directions. The arming chain is genuinely <em>driven</em>: the press moves the pawl, the pawl indexes the column, and every part downstream stands where the surface pushing it puts it — the simulation measures every one of those contacts closed on every test run. The ringing is not: the hammer\'s <em>tempo</em> while the alarm rings is authored, chosen rather than derived from the spring\'s torque, because deriving it would need a model of the spinning governor that real alarms use as a speed limiter. The model states that debt rather than hiding it.']:
    '诚实说明，两个方向都说。启动这条链路确实是被<em>驱动</em>的：按压带动棘爪，棘爪把柱轮推过整整一齿，其后的每一个零件都停在推动它的那个面所决定的位置上——每次测试运行，仿真都会把这些接触逐一测为闭合。响铃则不然：闹响期间音锤的<em>快慢</em>是写定的，是选出来的，而非从发条力矩推导而来，因为那需要为真实闹钟用作限速的风轮建模。模型把这笔欠账明说出来，而不是藏起来。',
