...
export default {
  ...
  ['Honesty note: in this simulation the spring is real arithmetic, not a prop. The wheel\'s inertia is computed from its drawn dimensions — about 85% of it in the rim, a tenth in the screws, the rest in the arms — and the hairspring\'s cross-section is then <em>solved</em> so that the square root lands exactly on the advertised rate. The ribbon that comes out of the solve is about 0.024 mm thick, inside the 0.02–0.04 mm range real hairspring stock is drawn in. The drawn amplitude used to be the one liberty here and no longer is: the balance swings the arc a real one does. What is still left out is the few degrees a real watch loses over a long run to friction in the escapement itself — this movement\'s fusee keeps the push level, so there is nothing to make the arc sag except losses nobody here has modelled yet.']: '诚实说明：在这套仿真里，游丝是真的在算，而不是一件道具。轮子的惯量由它自己画出来的尺寸称得——其中约 85% 在轮缘，十分之一在螺钉，其余在轮辐——游丝的<em>截面</em>随后被反解出来，使那个平方根恰好落在标称频率上。解出来的这条带子厚约 0.024 mm，正处在真实游丝钢带 0.02–0.04 mm 的常用区间内。上面画出来的摆幅曾经是这里唯一的自由发挥，现在不再是了：摆轮摆出的弧就是真实摆轮摆出的弧。仍然被省掉的，是真实的表在长时间运行中因擒纵机构自身的摩擦而损失的那几度——本机芯的宝塔轮把推力保持平直，所以除了还没有人在这里建模的损耗之外，没有什么能让这道弧垮下去。',
  ['Here the balance completes 2½ full oscillations per second, and each oscillation crosses the escapement twice — once each way — giving the 5 ticks per second of the previous entry. A healthy real balance swings roughly three-quarters of a turn each way, and that is now exactly what this one draws. At full speed it is a blur — a real balance is too, at 74 radians a second — so the time-scale slider is how you actually watch it: slow it down and the arc resolves into the swing the arithmetic describes.']: '此处摆轮每秒完成 2½ 次完整振动，而每次振动两次穿过擒纵机构——来回各一次——于是得到上一条目里每秒 5 下的滴答。真实状态良好的摆轮每侧大约摆过四分之三圈，而现在这一枚画出来的正是这个。全速下它是一片虚影——真实的摆轮也是，每秒 74 弧度——所以真正看清它的办法是时间倍率滑块：把它放慢，这道弧就会化解成算术所描述的那个摆动。',
