...
export default {
  ...
  ['A flat spiral does not quite pay that bet, and watchmakers have known it since Breguet. The spring\'s outer end is clamped to a fixed post, so the coils do not breathe evenly about the centre as they wind and unwind: the spring\'s centre of mass sits a little off the pivot axis — here about a tenth of a millimetre — and shifts as the spring breathes. The spring then leans sideways on the pivots, by a different amount at every amplitude. That is a rate error which varies with how far the balance swings — precisely the error isochronism was supposed to remove.']: '平游丝并不能完全兑现这个赌注，自宝玑以来制表师就知道这一点。游丝的外端夹在固定的外桩上，因此在卷紧与放松时，各圈并不会绕着中心均匀地呼吸：游丝的质心略微偏离轴尖的轴线——这里约十分之一毫米——并随着游丝的呼吸而移动。于是游丝对轴尖产生侧向的推挤，而每个摆幅下的力道各不相同。这就是一项随摆轮摆幅大小而变的走时误差——正是等时性本应消除的那一项。',
  ['The classical answer is the <b>overcoil</b>, Breguet\'s. Lift the last three-quarters of a turn clear of the body — here by under a third of a millimetre, barely more than the ribbon is tall — and curve it back <em>inward</em>, so the spring finishes above its own coils instead of out beside them. Get that curve right and the centre of mass lands on the pivot axis: the coils breathe concentrically, and the sideways lean nearly vanishes. This balance carries one. Its curve is not drawn by hand but <em>solved</em> — for the centre of mass landing on the axis, for the end coming to rest over the body of the spring rather than outside it, and for a bend no tighter than the one the ribbon already takes where it grips the staff.']: '经典的答案是宝玑的<b>末端曲线</b>。把最后四分之三圈抬离游丝本体——这里不到三分之一毫米，仅略高于丝带本身的高度——再向<em>内</em>弯回去，使游丝终止于自己圈层的上方，而不是旁边。这条曲线取得对，质心就落在轴尖的轴线上：各圈同心地呼吸，侧向推挤几乎消失。这只摆轮就带着一条。它的曲线不是手绘的，而是<em>解出</em>的——要让质心落在轴线上，要让末端停在游丝本体的上方而非其外侧，还要让任何一处弯曲都不比丝带在抱住摆轴处已经取的弯曲更急。',
  ['The raised curve is the shape the solve produced, traced at the scale of the drawing; the flat spiral beside it is schematic. Measured on this spring, at the swing drawn on screen, adding the terminal curve takes the sideways push on the pivots from about 4.7 to about 0.24 millionths of a newton — roughly a twentieth. At a real three-quarter-turn swing a residue survives, because the rule the curve is solved against is a small-swing statement: there it is about a quarter rather than a twentieth, and this page reports that rather than rounding it away. The centre-of-mass marks are drawn far larger than life, as the swing was in the previous plate — the real offset is about a tenth of a millimetre on a spring some 6 mm across.']: '抬起的那条曲线是解算给出的形状，按图的比例描出；旁边的平游丝只是示意。在这根游丝上测得：在屏幕所绘的摆幅下，加上末端曲线后，轴尖所受的侧向推力由约 4.7 降到约 0.24 百万分之一牛顿——大致是二十分之一。在真实的四分之三圈摆幅下仍有残余，因为曲线所依据的规则是一条小摆幅的论断：那里约为四分之一而非二十分之一，本页把它说出来，而不是四舍五入抹掉。质心标记画得远大于实物，正如前一图中的摆幅——真实偏移在直径约 6 mm 的游丝上约为十分之一毫米。',
  ['Lifted clear, curved back inside']: '抬起，再向内弯回',
  ['flat spiral']: '平游丝',
  ['with the overcoil']: '带末端曲线',
  ['centre of mass off the axis']: '质心偏离轴线',
  ['centre of mass on the axis']: '质心落在轴线上',
  ['the spring leans on the pivots']: '游丝推挤轴尖',
  ['the lean nearly vanishes']: '推挤几乎消失',
  ...
  ['Plate 2 slows one beat down to a few seconds and draws it from the lever\'s point of view, so the stones stand still and the wheel does the moving. Watch the right stone: the tooth\'s tip sits on its corner, slides down its slanted end as the stone lifts, and drops off — and the next tooth is already landing on the left stone\'s corner.']: '图版 2 把一次半振动放慢到几秒钟，并从擒纵叉的视角来画，于是瓦石静止不动，动的是轮子。看右边那颗瓦石：齿尖坐在它的角上，随着瓦石抬起沿斜端滑下，然后脱离——而下一枚轮齿已经落在左边瓦石的角上。',
  ['What the slant buys. The stone\'s end is cut so that the tooth\'s tip, travelling forward while the stone lifts, stays pressed on it for almost the whole step — the wheel only runs free for the last degree and a half before the other stone catches it. Cut the slant the other way and the tip would leave the corner into thin air, the wheel would spin the whole step unloaded, and the balance would get nothing. Two things here are honest simplifications: the lever\'s swing is drawn as the simulation poses it, and the little backward twitch of the wheel at the start of each beat is a posed gesture rather than a measured recoil.']: '斜面换来什么。瓦石的端头切得让齿尖在瓦石抬起时一路向前，几乎整个一步都紧贴着它——轮子只在另一颗瓦石接住它之前的最后一度半里自由转动。把斜面切反，齿尖会从角上脱离到空中，轮子整整一步都空转，摆轮什么也得不到。这里有两处诚实的简化：擒纵叉的摆动按模拟所摆的姿态画出，而每次半振动开始时轮子那一下小小的回退，是摆出来的姿势，不是测出来的回退。',
  ['One beat, slowed down']: '一次半振动，放慢来看',
  ['unlock · the stone lifts, the wheel twitches back']: '解锁 · 瓦石抬起，轮子微微回退',
  ['the stone about to catch']: '即将接住的瓦石',
  ['the stone letting go']: '正在放开的瓦石',
  ['the lever\'s frame, in the machine\'s proportions: stones still, wheel moving; the dot is a touch']: '擒纵叉的视角，按机器本身的比例：瓦石不动，轮子在动；圆点表示接触',
  ['beat']: '半振动',
  ['locked · the stone holds the wheel']: '锁住 · 瓦石扣住轮子',
  ['impulse · the tip rides the slant']: '冲击 · 齿尖沿斜面滑行',
  ['drop · the wheel runs free to the other stone']: '落差 · 轮子自由转到另一颗瓦石',
};
