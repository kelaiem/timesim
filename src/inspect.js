...
export const MECH_GRAPH = {
  ...
  drive: [
    ...
    ['Keyless works', 'Motion works'],       // SETTING: TODO 151's (d) landing put the cap on the
                                             ...
                                             // contact now (EXPECTED_CONTACT_FLOORS). TODO 155 closed
                                             // the residue: the cap is posed from the wheel it meshes,
                                             // going train and hand-set together.
...
// §36 APPLY — the route's own row, pushed rather than written inline because
// it exists only when a route does. The predicate is ROUTE_SPEC ITSELF, not a
// copy of its test: checkGraph fails in BOTH directions — a declared unit that
// was not built, and a built unit that was not declared — so the row's
// condition has to be exactly the condition main.js builds on, or one of those
// two failures is guaranteed on some spec.
//
// SUPPORT ONLY, and no drive edge. An applied route is static modelled stock:
// the plate holds it (through a bore it was cut for, or a bush footed on it —
// routeApplySolve refuses a route with neither, so this claim is always real
// metal within SUPPORT_TOL), and nothing turns it. A drive row here would be
// the simulation fiction CLAUDE.md names: an edge with no force behind it.
if (ROUTE_SPEC) MECH_GRAPH.support.push([ROUTE_UNIT_NAME, 'plate']);
...
const EXPECTED_PAIRS = [
  ...
  ['Keyless works', 'Motion works'],         // SETTING: TODO 151's (d) landing put the cap on the
                                             ...
                                             // contact now (EXPECTED_CONTACT_FLOORS), and TODO 155
                                             // poses it from the wheel it meshes
...
export const INTRA_UNIT_CONTACTS = [
  ...
  { unit: 'Keyless works', a: 'settingCap', b: 'settingCapArbor', why: 'TODO 151\'s (d) landing — the setting cap on the vertical arbor that carries it up from the cap corner (mwCornerCap) — one arbor, two meshes' },
  { unit: 'Keyless works', a: 'mwCornerFootIn', b: 'settingRise', why: 'TODO 151\'s (d) landing — the foot corner\'s inboard bevel, keyed to the tilted rise leg B→E' },
  { unit: 'Keyless works', a: 'mwCornerFootOut', b: 'settingStub', why: 'TODO 151\'s (d) landing — the foot corner\'s outboard bevel, keyed to the horizontal stub E→D' },
  { unit: 'Keyless works', a: 'mwCornerCapIn', b: 'settingStub', why: 'TODO 151\'s (d) landing — the cap corner\'s inboard bevel, the stub\'s other end' },
  { unit: 'Keyless works', a: 'mwCornerCapOut', b: 'settingCapArbor', why: 'TODO 151\'s (d) landing — the cap corner\'s outboard bevel, keyed to the vertical arbor up to the cap' },
  ...
  { unit: 'Alarm switch', a: 'alarmColDriver', b: 'alarmColPawlSpringStud', why: '§192: the blade spring\'s anchor stud riveted into the driver\'s THIRD ARM, cut to reach it. The name has history: §163\'s row with these labels described a stud that measured 4.347 clear of the driver in every direction — a declared joint that stated something false and excused the pair from the sweep that would have caught it, which is why §169 replaced the blade with a coil and §182 now audits every row in this table against the metal. The stud returns with its ground actually cut: hull metal under it by construction, asserted at the build, and this row measured every battery run' },
  { unit: 'Alarm switch', a: 'alarmColPawlSpring', b: 'alarmColPawlSpringStud', why: '§192: the blade clamped to that stud — the ground the restoring force reacts against, and the clamp face its free length is measured from' },
  { unit: 'Alarm switch', a: 'alarmColPawlSpring', b: 'alarmColPawlTail', why: '§192: the blade\'s free end bearing on the pawl\'s tail flank — the contact that closes it. The tail is its own body because the arm is CLIPPED off the pivot bore (the post is wider than a 2w arm, so a centreline through the origin would put it through the arm\'s flanks whatever hole was cut); bear station at the boss\'s edge, free length the greater of the drag budget\'s floor and its own strain limit\'s, and the tick\'s frame law holds the built tip on this flank at every pose' },
  ...
  // midpoint of either lands in the other's metal and point sampling saw
  // nothing. They are joints, not fouls, and were triaged as such rather than
  // assumed — over the whole pose net each pair's shared volume is CONSTANT
  // (1296, 1284, 1296 grid points, unchanged across 42 poses) and the shared
  // centroid sits 0.0016–0.0026 u off the post's own axis, which is a part on
  // an axis, not a part crossing one.
  //
  // What is missing as metal is the bore: the BUSH is modelled (ringGeo 0.15
  // inner against the post's 0.14, the 0.01 running fit this ring turns on)
  ...
  // TODO 95/94: the finger disc on its arbor. Triaged, not assumed — shared
...
export const MESH_PHASE_WAIVERS = {
  ...
  // `setting fold minute wheel ⇄ setting cap` IS GONE — TODO 155 paid it: the
  // fold is now posed from the wheel the cap meshes, going train and hand-set
  // together, rather than from handSetOffset alone. 42.951% → 0.129%, the
  // stale gate named it.
};
...
export const TRANSMITS_WAIVERS = {
  ...
  // different reason.
  // `setting fold minute wheel ⇄ setting cap` IS GONE too — TODO 155's fix
  // (the fold posed from the wheel it meshes) reads MISMATCH → ok −3.75 on
  // both declared inputs, the stale gate named it, MESH_PHASE_WAIVERS' twin
  // gone with it.
};
