...
export const INTRA_UNIT_CONTACTS = [
  ...
  { unit: 'Alarm switch', a: 'alarmColDriver', b: 'alarmColPawlSpringPin', why: '§169: the pawl spring\'s anchor pin riveted into the driver, on the post arm\'s own centreline — where the web is asserted at the build to be wider than the pin plus a margin. This row REPLACES §163\'s `alarmColDriver ⇄ alarmColPawlSpringStud`, which said the same thing about a stud that measured 4.347 clear of the driver in every direction: a declared joint is a claim, and that one both stated something false and excused the pair from the sweep that would have caught it' },
