...
// Brand crown (§27) — the barrel is lathed to the house double-lobe
// hourglass silhouette and the face carries makeBrandMark's ∞ in relief
// (see makeCrown in geometry.js). Same bodyR/bodyH as the knurled crown
// it replaces: the redesign stays inside that build's PROVEN swept
// envelope (asserted in the builder), so every standing clearance row
// holds without re-derivation.
...
const alarmSetI2Spin = new THREE.Group();
{
  const mk = (spin, pos, teeth) => {
    ...
    const idler = G.makeGear({ module: ALARM_SET_MODULE, teeth, thickness: 0.25, boreR: 0.5, spokes: 4, material: MATS.brass, bevel: false, hub: false }); // crisp + hub-less: the dial-sheet budget (see the setting wheel)
...
// consumer of makeBrandMark's ∞, same lathe silhouette at 4.0/3.4.
