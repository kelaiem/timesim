...
export const BATTERY = [
  ...
  { name: 'oscillator', opts: {},
    ...
    note: (r) => `implied ${r.impliedHz} Hz vs spec ${r.specHz} Hz, ribbon ${r.spring.h_mm.toFixed(4)} mm (stock ${r.spring.windowMm[0]}–${r.spring.windowMm[1]})`
      + (r.breathing ? `; clamp ×${r.breathing.clampRatio.toFixed(4)}, pivot ${r.breathing.peaks.pivotForce_mN.toFixed(4)} mN @${r.breathing.peaks.ampDeg}°, stress ${r.breathing.peaks.stress_MPa.toFixed(0)}/${r.breathing.fatigue_MPa} MPa` : '') },
