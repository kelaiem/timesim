...
const MUTANTS = [
  ...
  { kind: 'subject', name: 'the link\'s side in its wrap', file: 'src/main.js',
    find: '    + y.y * (P.barrel.y - (joints[0].y + joints[1].y) / 2)) >= 0 ? 1 : -1;',
    to:   '    + y.y * (P.barrel.y - (joints[0].y + joints[1].y) / 2)) >= 0 ? -1 : 1;',
    note: 'the run\'s path is right and the LINK is mirrored inside it — the §124 lean tips away from the flank and the seat crowns name the outer edge, both off one frame' },
  ...
  { kind: 'subject', name: 'the keyless chain\'s spur rate', file: 'src/main.js',
    find: 'const SPUR_RAD_PER_TURN = -MOVEMENT_SENSE * 2 * Math.PI;',
    to:   'const SPUR_RAD_PER_TURN = MOVEMENT_SENSE * 2 * Math.PI;',
  ...
  { kind: 'subject', name: 'the setting fold\'s net sense', file: 'src/main.js',
    ...
    note: 'HAND_RAD_PER_SET_RAD, both rawSetOffset sites and the dropZ assert all read this one constant — flipping it alone (leaving MW_FOLD_SPIN\'s own built sense at −1) should disagree with the fold\'s own measured mount, which is exactly what the dropZ assert exists to catch' },
];
