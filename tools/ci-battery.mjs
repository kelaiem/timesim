...
const COSTS = {
  ...
  // cost is rows × inputs × halvings rather than rows × poses. MEASURED at
  // 10.4 s and 5.1 s on the container that ran the landing battery — the first
  // guesses here were 34 and 22, which is the kind of stale number this column
...
const ONLY = (() => {
  ...
  // The legal key space is every DECLARED check and every slice a check
  // declares — read from BATTERY, not from COSTS. Those two used to be the
  // same set and are not: since §122 the `clearances:*` and
  // `expectedContacts:*` slices carry no COSTS row at all (they project from
  // pose count and are labelled `projected`), so a COSTS-keyed check here
  // would reject the very slices this flag exists to select.
  const legal = new Set();
