...
| 115 | CLOSED | Issue #327, and wider than reported: EVERY display the going train drove counter-rotated the wheel it was keyed to (fourth wheel −2.094395 against the seconds hand and the heart cam at +2.094395, centre wheel −0.034907 against the hour wheel at +0.002909, equal and opposite to the last digit — a SIGN, not a phase). Two halves, and neither alone was a fix: the dialFace seam, where four dial-side displays now take the NEGATED movement-frame angle and one MOVEMENT-frame part stops taking it negated; and the train's ABSOLUTE sense, now one factor in `escapeAngle` off `MOVEMENT_SENSE` (`layout.js`), the single declaration every direction-committed cut derives from. **The dial-side numbers did not move** (2.094395, 0.002909) — the displays run as they always did and the wheels under them stopped disagreeing, which is the proof that the dial was right and the train was backwards. Nine handed cuts and three winds re-derived where they are cut; the pallet fork's commitment turned out to be a MIRROR of its whole stone assembly — body side, slot, head walls, tooth-motion tangent, face normal — not a sign, and its own draw-torque assert (invariant under a true mirror, so never parameterised) caught every partial attempt. A tenth commitment nobody had listed, the escape wheel's PHASE, moves no vertex and so read NO-OP to the mutation probe until its metal hash learned to carry world matrices; it has a guard now that reads the club tips out of §83's cut outline. `probe-coaxial-sense.mjs` PASSes. **Five regions: the escapement, the dial seam, the motor, the keyless winding chain and §47's winding arrest.** The last reverses which way you turn the CROWN to wind — a consequence of the gearing, not a preference — and is walked through the keyless gears from `windLocalAt`'s own law so no link's sense is free. TODO 50's saw coupling had to be re-cut, and taught the item a new lesson: a MIRRORED pair does not seat where the original does (its clear window is the backlash's other end; unclocked it measures tip-on-tip, 0.1699 buried, exactly one toothH), and giving the metal and the ride law DIFFERENT clockings still passed every column of `stemClutchHandoff` with the camming pose 0.0256 off contact — inside the gate, riding a coupling that is not there. One number now, in both. The POSE TABLES were direction-committed too — `stemSlip`'s axis and the handoff poses swept the driving direction on a reversed movement, measuring poses the watch never reaches. **The FIFTH region landed too**: §47's winding ARREST was clocked against a coil and a cone this item moved, and its four red rows came apart into three separate defects rather than the one clocking error the item predicted. Which side the lug arrives from is the movement's, and the beak tab's stock and the lug's closing sweep are that one commitment read from the two members — flip either alone and the pair meets stock-on-stock (−0.3267). The pad is a plane on a LEVER, so what a coil demands of it is a ROTATION, not a radius: the face turns 0.35 rad about its stud and swings its own width sideways, so it stands ℓ·tanψ off where the ray reading says, and only a coil whose proudest point sits ON the ray makes the two agree — the wrap's LINK PHASE decides that, and reversing the movement moved the point to 0.0908 of a 0.107 half-window. `psiDemand` asks for the rotation in closed form, as a LEAST fixed point (which links the face is over moves with the rotation: seated it spans world-tangential [−0.35,+0.35], thrown [+0.09,+0.72]), and the rest radius is BISECTED against it with the stud inside the loop, because stepping limit-cycled 1.13e-3 off the designed throw. And the third defect the item never predicted: the chain's LINK was mirrored inside a wrap whose path was already right — ŷ = k̂ × t̂ takes its sign from the tangent, so §124's inboard lean AND the seat crowns that declare where it beds both flipped, the float row reading 1.286–1.491 against 0.25 at every reserve while the burial row stayed green at 0.188 because the outer edge had taken the inner edge's place. Correcting the lean alone makes it WORSE (2.348 — a correctly leaned link measured on its outer edge), which is the tell that the frame is the one source; t̂ and ŷ now negate together off one geometric test and the float row reproduces the reference build to the last digit at all 60 samples. And a FOURTH, which is why that one cost two landings: `linkOuterPtsNear` — the sampler the arrest's whole fold is solved against — carried its own verbatim copy of that frame law, so it went on modelling the mirrored link after the metal was fixed and the pad arm solved against a chain that no longer existed (`expectedContacts` Winding arrest ⇄ Chain 0.1325 against its 0.15 floor). One `chainLinkFrame` above both readers now, and the fold re-solves itself: the arm's station goes out, PAD_T grows with it, 0.1703. The recurring defect is not a wrong sign, it is a SECOND COPY. `probe-direction-guards.mjs`: 17 commitments, 0 silent. The LAST red row was an instrument, not a mechanism: `penetration`'s Keyless works ⇄ Winding clutch reported 0.17 = one toothH because an MTV cannot separate two INTERLEAVED combs, and the +1 reference read green on the same geometry only because its axis sampled poses where the rings never touched. `sawRideDepth` bisects the pair's OWN free coordinate — the smallest slip rotation that clears the intersection, as arc at the mean tooth radius — and reads 0.007 against the unchanged 0.03, with a LINEARITY control in probe-50-clutch.mjs (pre-rotate 0.02/0.05 rad, the reading moves by exactly that; the other way it clears). Two earlier measures came back clean while measuring nothing, both in the SKILL.md catalogue: an axial ray meets each ring's engaging face from behind and FrontSide reports no backface, and a bounding box's corner is not a ring's radius. The beak now KISSES its lug at 6.0e-9 instead of biting it, which is why `inspection` stopped listing that pair as contacting. Residue, unchanged: the fusee groove still has no instrument — its hand is read from the generator's emitted points, not from the welded metal — and `penetration`'s `Keyless works ⇄ Winding clutch` 0.17 stays red as a MEASUREMENT problem (one `toothH`, what an MTV reports for interleaved combs that touch at all) |
...
- **The setting wheel ⇄ I1 mesh above is now MEASURED at 37.44%**, so it is no
  longer "reported by eye, not instrument-confirmed" — and it is NOT this
  entry's half-pitch idiom at all. **Item 117** took it apart with a second
  instrument: that chain does not TRANSMIT, which is the prior question to
  phase. One sign is fixed there (the arbor pinion's back-drive); the rest is
  a three-law contradiction, filed rather than guessed at. The lesson for this
  entry is that a pair can be perfectly phased and not be geared at all, so
  `probe-mesh-transmission.mjs` belongs beside the phase probe whenever a site
  here is re-opened.
...
`ALARM_HAND_Z`'s derived lane (since item 119 the hour plane's offset
itself DERIVES from that lane); the minute hand's 2.3 was NOT datumed
...
says report, triage, then gate, and nobody has looked at these. **They are
item 109's now** (2026-08-28), on item 55's rule: a closed item is a bad
place to keep live debt, and this catalogue proved it by drifting in three
rows while nobody re-ran it. Six of the
...
### Finding 5 (2026-08-26, MEASURED — **CLOSED by §183**) — the lock's READ is posed too: the column cannot block the lever it is declared to block
...
#### CLOSED by §183 — the read is a ROCKER, because every station with gain was illegal or taken

The search above refuted moving the beak, so the chain is FOLDED instead. The
beak stays at the free, parity-correct station it already had — declared
contact, parity and the wheel's placement all untouched, and
`ALARM_LOCK_BEAK_OFF` is 0 and named to say so — and gets its OWN pivot, placed
where the read is radial: `Q = B0 + arm·n̂`, with the arm the smallest that
stands its post clear of the saw (§171's rule).

| | |
|---|---|
| `ALARM_ROCKER_ARM` | **3.1400** — Thales, so radial gain is 1 by construction |
| `ALARM_COL_RCHAM` | arm × `ALARM_LOCK_LIFT` = **0.3047** against a 0.6327 ceiling |
| nose centre, column → gap | **5.9800 → 5.6754** (was 0.00114 of excursion) |
| finger, seated → clear | **3.5000 → 3.9667**, both solved stations, exactly |

**The pillars gained a radial cam cut by the SAME function that ramps the top**
(`rOutAt` beside `profileAt`, `rCham` defaulting to 0 so no other caller moved)
— TODO 20's invariant extended to the second follower rather than duplicated
for it. **The pin is on the P–Q line** because Q stands off to the side to clear
the saw and any other placement leaves the two velocities nearly orthogonal; a
radial slot takes it, §163's idiom one tier down. **Nothing new is sprung** —
the §102 blade presses the beak onto its cam through the located pin, declared
`two-way`. And **the lever's pose is SOLVED from the cut** at every wheel angle,
§173's jumper convention, so `ALARM_COL_RCHAM` finally reaches the lever where
a normalised amplitude never did.

Four errors the rule-6 asserts caught, one boot each: the pin opposite the beak
arm rather than on the P–Q line (lever 0.186 off its rest, travel −0.031 for
+0.097); `rPin` from the first-order ratio (0.09613 for 0.09703, because both
members swing on arcs); a BOX beak laid along a TANGENTIAL arm, which
`alarmHandoffs` refused at −0.3022 and which is a round `riderNoseR` nose now;
and a TEMPORAL DEAD ZONE paid for the fourth time in this file.
...
**A third instance, found 2026-08-28 by TODO 109's bearing scan, and again by
accident.** The `Alarm release feeler` row `BoxGeometry#8 ⇄ BoxGeometry#9` —
*"the §29 tail RUN sliding through its cheek mid-guide (kiss at cam poses) —
§103's second guidance station"* — describes a MID-SPAN GUIDE. Measured off the
built tree, `#9` sits at run-local x **+4.5673** on a run whose extent is
±4.5673: the far END, and it is the riser that carries the beak down — a rigid
corner of the same lever, not a guide the run slides through. Tier A cannot see
this: the two parts genuinely touch, so the row passes `DECLARED_CONTACT_REACH`
while describing the wrong KIND of joint, which is precisely what tier B is
for. It cost TODO 109 a detour — the row is the only evidence in the repo that
the tail run has a second support, and a slenderness declaration was nearly
written against it.

**It has stated something false twice before that, and both times were
accidents.** §169
...
## 109. §54's seven unwaived λ rows are live debt parked in a closed item — NAMED, priced, and untriaged

TODO 78 registered `checkSlenderness`, gave it the per-free-length
measurement §54 had promised, and printed what it found: **nine rows over
the λ 30 ceiling, seven of them unwaived**. It closed with the residue
written down — *"seven unwaived rows are open debt this item does not
close … nobody has looked at these. Do not add waivers to green this"* —
and the roadmap records the same ownership (`§156`: "TODO 78 owns the §54
catalogue and TODO 79 the overhang").

**That is the defect this item exists to correct.** TODO 78 is CLOSED, so
its catalogue is a snapshot in a document nobody re-runs, and item 55 already
established the rule this breaks: *a closed item is a bad place to keep live
debt* — which is why its own residue became item 56. Two consequences, both
measured below rather than argued: the catalogue has **drifted** since it was
printed, and six of its nine rows were addressed positionally as `(unnamed)`,
which is a sufficient explanation for why nobody has triaged them. A row that
says `Hack rod / (unnamed)` does not say WHICH bar, and `Keyless works` holds
**40** meshes sharing that non-name.
...
exactly what fails here: it silently returns the first `(unnamed)` mesh in
the unit and describes the wrong part. All nine rows matched exactly one
mesh; the pools the name alone left were 40, 22 and 8 for the three still
anonymous, which is the control that says the matching did work.

| unit / mesh | what it actually is | λ | k, free cantilever | waived |
|---|---|---|---|---|
| `Alarm link` / `alarmLinkShaft` | the lay shaft's rod-end overhang | **127.6** | 36 N/m | TODO 16 |
| `Alarm release lifter` / `alarmLifterRun` | the lifter's 23.3 u run, 0.327 × 0.317 stock | **71.3** | 16.5 N/m | — |
| `Hack rod` / `rodSegOut` | §137's elbow rod, outer segment, r 0.35 | **64.8** | 48.8 N/m | — |
...
623 meshes counted, 34 exempt by kind, control PASS (13 cases), 0 malformed
and 0 unsupported declarations, 0 stale waivers.

**Three rows have moved since TODO 78 printed the catalogue**, which is what
a snapshot in a closed item does: `Keyless works` 40.4 → **41.2** (k 190.9 →
180.4), `Alarm link / alarmLinkRod` 31.4 → **34.8** (k 367.3 → 270.3), and
both `Hack rod` rows by a tenth (65.0 → 64.8, 37.7 → 37.5). Not
investigated here — the point is that the numbers are alive and the document
holding them is not.

**Four of the six anonymous rows became nameable without anyone noticing**:
§137 named the elbow rods' segments `rodSegIn` / `rodSegOut` because its
transfer rows needed to name their members, which fixed three of these rows'
addresses as a side effect. Three remain anonymous, and all three are listed
above with the geometry that makes them greppable.

### What every unwaived row has in common, and it is one thing

**None of them declares `userData.bearings`.** Eight of the nine rows are
measured over their WHOLE STOCK; only `alarmLinkShaft` declares where it is
held. So for seven parts the reported λ is not a free length at all — it is
the same defect TODO 78 diagnosed for the alarm link and fixed *only* for the
alarm link.

That is the first triage question per row, and it is cheap: **is there metal
...
Per row, in this order, and the order matters because step 1 can retire a row
without anyone touching metal:

1. **Declare the bearings that exist**, and let the check re-measure the free
   lengths. Metal first — `supportAt` will refuse a station that is not held.
   Expect this to *reveal* an overhang as often as it retires a row: §54
   charges an overhang `SLENDER_OVERHANG_K` = ∛16, so a bush near one end can
   read worse than the undeclared whole stock (that is exactly how the alarm
   link got to 127.6).

   > **DONE, and the step's own premise is REFUTED. It retired nothing and it
   > reddened two.** See *Step 1, measured* below: the paragraph above guessed
   > "as often as"; measured over all seven rows the answer is *always*, and
   > for five of them there is no bearing to declare at all.
2. **Where the free length is real, price the section.** The probe prints
   what each row's own governing length needs to reach its ceiling, which is
   ...
   reads. Five of the seven are inside ×1.4 — this is not a redesign for
   most of the population.) A section that grows is a P3 question the moment
   it lands: every one of these bars is routed through a corridor somebody
   cleared, so each re-section owes a pair sweep and, for the two rods,
   `ELBOW_E_MAX`'s own scan.
3. **Waive only what has a fix path**, citing a TODO — never to green the
   report. TODO 78's instruction stands.

### Step 1, measured — it retires nothing, and two rows get worse

`tools/probe-bearing-candidates.mjs` walks each row's long axis and asks, at
every station, what metal contains the point and **which unit that metal is
in** — because `supportAt` looks for a bearing only inside the declaring
mesh's own labelled unit, and a station with no same-unit metal FAILS the
battery. The scan settles all seven rows:

| row | what holds it | declarable? |
|---|---|---|
| `Alarm release feeler` tail run | its own jog, a built-in root, **same unit** | **yes — landed** |
| `Alarm crown` stem | `alarmBush`, a bored boss at `plateR − 2`, **same unit** | legal, but withdrawn — TODO 110 |
| `Hack rod / rodSegOut` | `Stop lever / stopCrankRodPin` — **another unit** | no |
| `Hack rod / rodSegIn` | `Setting lever / hackRodPin` — **another unit** | no |
| `Reset rod / rodSegOut` | `Reset hammer` — **another unit** | no |
| `Keyless works` traverse rod | **nothing at all** over 28.84 u | no |
| `Alarm release lifter / alarmLifterRun` | nothing but abutting run segments | no |

**What landed — one of the two, and the other one is why item 110 exists.**
The station is written as the arithmetic that places the metal holding it, so
moving the jog moves the free lengths with it, and it is re-measured off the
built tree afterwards:
...
not move, so its station is a pose rather than a place on the metal — and
`alarmCrownPullT` is EASED, so `resetInputs()` zeroes the variable while the
scene follows only on a later tick with real dt. `start()` runs its check in
the same microtask as that reset, so `supportAt` reads whatever pose the
previous check left: after `alarmHandoffs` the stem stands 5 u out and the
declaration lands in air. The battery caught it — 35/36, one row — where the
focused probe had passed it twice, because a probe's `page.evaluate` calls
have rAF frames between them and the harness does not. **That is TODO 110**,
filed rather than worked around; the stem's real λ 76.6 is recorded at the
metal, in `main.js`, where a fix would look.

Neither number is new debt: both are the old debt reading true. Whole-stock λ
treats a bar as a SPAN, and neither of these is one — a stem held by a single
bush 25.52 u from its inner end, and a tail run built into a jog at one end
with the beak hanging off the other, are both cantilevers, which §54 charges
`SLENDER_OVERHANG_K` = ∛16 for. `unsupported` and `malformed` stay 0 and the
control still passes 13 cases, so the declaration is legal as well as true.

One caveat on "derived", stated rather than glossed: the stem's station comes
from `ALARM_STEM_BUSH_DIST − ALARM_CD − alarmStemLen/2`, every term of which
already placed metal. The tail run's comes from the jog's own station, and
that — `TAIL_RUN_ROOT_X` = 0.35 — is an INHERITED literal this landing named
rather than derived. Naming it buys only that the jog and the station citing
it cannot drift apart; it is a §86-class number for whoever derives it.

**Three things the scan established that guessing would not have:**

- **The rods cannot be declared, and would not benefit.** Their working ends
  are pins in other units, so `supportAt` refuses them — and the refusal costs
  nothing, because a pin-ended two-force link's free length IS pin to pin.
  The rows are already measuring the right length. (`SLENDER_OVERHANG_K`'s
  end-face case means a station there would change nothing anyway.)
- **The setting arbor is supported by NOTHING.** 28.84 u — 10.9 mm — between
  its two bevel corners, with only the corner gears and the two vertical legs
  as same-unit neighbours. A real keyless traverse of that length runs in a
  cock. So this row's fix is to ADD metal in position space, not to declare
  any: it is the one row here that is a layout landing.
- **The stem's station is a POSE, and rest is the worst of them.** The crown
  slides the stem through a bush that does not move, so the bush walks along
  the metal; pushed in — the pose `start()` measures — the run inboard of the
  bush is at its longest. Written at the declaration.

**Two ways this landing was nearly wrong, both caught by measuring:**

- The λ 35.1 row is **not** the feeler's arm. It is the §29 step-4 TAIL RUN,
  which happens to carry the same 0.26 × 0.10 section; the arm is a different
  mesh two units of z away. Attributing an anonymous row to the builder that
  *looks* right is exactly the failure this item was filed to end.
- The station was first derived through `_alarmRimD` — the ARBOR's rim
  distance — where the stem's own length is measured from `ALARM_CD`, the
  spinner's pushed-in rest radius, 5 u further in. The declaration landed in
  thin air and `unsupported` caught it: 1 failing row, exactly as designed.
  A support that does not exist as metal is the one thing this mechanism
  gates, and it earned its keep on its first use outside the alarm link.

**Found in passing, filed not fixed (TODO 104's class):** the
`INTRA_UNIT_CONTACTS` row for `Alarm release feeler` — `BoxGeometry#8 ⇄
BoxGeometry#9`, *"the §29 tail RUN sliding through its cheek mid-guide … §103's
second guidance station"* — describes a mid-span guide that is not there.
...
- **Not the alarm link.** Both `Alarm link` rows are waived to TODO 16, whose
  fix is roadmap §156's joint re-solve (three bearings at t ≈ 5.7 / 17.2 /
  28.8, r ≥ 0.1926) and whose overhang diagnosis is item 79. This item leaves
  those two rows alone; it exists for the seven the catalogue does not own.
- **Not a request to gate `slenderness`.** §54's report covenant holds (§40),
  and TODO 78's reasoning is unchanged: rows land red on arrival, and
  arriving as a gate is how a check gets switched off. What gates today —
  control, malformed, unsupported, stale waivers — is the right bar until the
  seven rows are triaged. Gating the λ rows is a *later* landing, and only
  after this one.
- **Not a load-path audit.** λ is a geometric ceiling. Two of these rows have
  their loads already computed by §137 Gate A (reset **0.83 mN**, hack
  **0.18 mN**, and the δ ≤ 0.1 × stroke standard breaking at 7.2 mN on the
  hack rod), and that arithmetic is a legitimate input to the triage — but a
  row is not retired by being lightly loaded today. Item 63 owns the force
  paths; this item owns the sections.

Probe: `tools/probe-slenderness-residue.mjs` (a REPORT — it prints and exits
0). It is also the reproduction: re-run it before believing any number above,
...
`start()` opens every check with `clock.resetInputs()`, and CLAUDE.md rests a
whole tranche of the harness on what that buys:
...
> invariant and is the bug.**

**For EASED state it does not hold.** `resetInputs()` assigns the state
VARIABLES (`alarmCrownPullT = 0` among them). The SCENE follows on the next
tick with real dt, because the ease is integrated in `tick()` — the trap
CLAUDE.md already documents one level down: *"`setPose` ticks with zero dt, so
anything eased (the minute jumper's snap, `crownPullT`) cannot move under
it."* `start()` runs its check in the same microtask as the reset, so no
frame intervenes, and a check that READS THE LIVE SCENE rather than posing it
measures the previous check's eased pose.

### Measured, with a 37-second reproduction

`checkSlenderness`'s `supportAt` is such a reader: it places a declared
station through `mesh.matrixWorld` and asks which sibling's box contains it.
With a bearing declared on the alarm stem — legal, supported, and correct at
rest:

| after | alarm stem, world x | `unsupported` |
|---|---|---|
| virgin boot | 30.2618 | 0 |
| `alarmHandoffs` | **35.2618** | **1** |
| `resetInputs()` **plus a frame** | 30.2618 | 0 |

```
node tools/ci-battery.mjs --only slenderness           --shards 1   # 8/8 PASS
node tools/ci-battery.mjs --only alarmHandoffs,slenderness --shards 1   # 8/9, slenderness FAILS
```

The full battery reproduces it as 35/36: on the shard that scheduled
`alarmHandoffs` immediately before `slenderness`, the stem is 5 u out.

**Why no probe caught it.** A probe drives the page through separate
`page.evaluate` calls, and rAF frames run between them — so the ease has
always relaxed by the time a probe measures. `tools/probe-slenderness-residue.mjs`
passed this declaration twice. The battery is the only context that reads
inside the same microtask, which is exactly the context the invariant is
claimed for.

### What it does NOT mean

Not every check is exposed. A sweep calls `setPose`/`enterAxis` per pose and
so establishes its own; λ itself is geometry-local and did not move. The
exposure is the narrow class of **checks that read the live scene without
...
1. **Settle the ease inside `resetInputs()`** — assign the eased quantity's
   ...
   change, and the one that makes the invariant true as stated. Wants a list
   of every eased pair first.
2. **Have `start()` step once with real dt after the reset.** One line, but it
   advances `tau`, which several checks read — a fix that moves other
   reports is a worse defect than the one it closes.
...
## 114. Three sites still name the alarm barrel as the movement's back-most metal — §112 moved it — CLOSED

**CLOSED 2026-08-30 by §187.** All three catalogued sites corrected, plus a
FOURTH the item did not catch (the back-stack comment's "0.6 mm of
clearance (the alarm barrel is fixed metal)" — the very line whose
arithmetic §187 rebuilt). The deeper fix is structural: the back stack no
longer derives from any NAMED part at all — `BACK_ENVELOPE` measures max z
per radial bin at boot, the movers boot cannot see are declared
(`BACK_SWEPT_ALLOWANCE` / `BACK_SWEPT_REGIONS`), and `probe-back-envelope`
in CI fails any bin the declaration under-states — so the class of error
this item records (prose crediting a part the metal outgrew) has nothing
left to attach to: the §39 comment now names the tower AND the measurement
that will outlive it. (The item's own line numbers had drifted by the time
it closed — main.js:26542/34482 were ~26776/34770 — which is its point
made small.)
...
## 115. The going train counter-rotates every display it drives — CLOSED

**What landed is at the end of this item (THE LANDING); everything before it is
the measurement that decided what had to. All five regions are landed — the
escapement, the dial seam, the motor, the keyless winding chain and §47's
winding arrest — and the last row standing turned out to be an instrument that
could not measure its own claim, so it was replaced rather than waived; that is
the section at the very end.**
...
nothing. `tools/probe-handedness.mjs` asks one question of each — does the
cut have a MIRROR AXIS, since a cut that does works the same driven either
way — and searches every candidate axis rather than looking for a named
feature. Residual as a fraction of each profile's own radial swing, 0 being
a perfect mirror:

| part | best axis | runner-up | verdict |
|---|---|---|---|
| *control* — a gear (setting wheel) | **0.00000** | 0.01113 | symmetric, as a generator cuts it |
| *control* — a ratchet saw (alarm arbor) | **0.15309** | 0.15326 | handed, as `makeRatchetAndClick` intends |
| alarm release track + notch | **0.00000** at 179.9° | 0.00058 | SYMMETRIC |
| heart cam (seconds reset) | **0.00000** at 179.9° | 0.00056 | SYMMETRIC |
| minute jumper star | **0.00654** | 0.09767 | SYMMETRIC |
| minute jumper beak | worst point misses its reflection by **0.0000%** of reach | — | SYMMETRIC |

The two controls separate by 758566×, and the bar is their midpoint rather
than a number chosen here. The runner-up column is the second reading worth
having: a gear has a mirror axis per tooth, so its runner-up is small too,
while the notch and the heart have exactly ONE axis each — which is what a
single working feature looks like, and confirms the axis found is the notch
rather than an accident.

**The item's own suspicion is refuted by that first row.** It read "the disc
tracks the hour wheel, so its ramp is direction-committed". There is no ramp:
the notch is the ABSENCE of track (`main.js:13358-13367`), the track is
extruded `bevelEnabled: false`, and the gap is centred on θ = 0 with square
radial ends. Nothing there prefers a side.

The LAWS were swept too, since a symmetric cut read by a handed law is still
handed. Response either side of its working point, which is taken as the
sample furthest from the response's MEDIAN (the alarm's pin sits at 0 and
peaks at the notch; the jumper's lever sits seated and moves away at a tooth —
one rule, both directions):

- alarm pin drop: worst |Δ| **0.00051**, 0.51% of the signal and **0.30×** the
  sweep's own sample step — EVEN.
- jumper lever angle: worst |Δ| **0.00019**, 1.17% of the signal and **0.95×**
  the sample step — EVEN.
- the heart's law is READ, not swept, and marked so in the probe: the tick
  takes `Math.sign(off)` against `heartFreeAngleAt(d)`, a table of DISTANCE
  alone, so it has no side to prefer. Its METAL is measured above.
...
The same probe's CENSUS tier sweeps every authored outline in the movement
(192 cuts) with the same mirror search, so the inventory stops being a list
someone wrote down and becomes a population. Against the same controls, bar at
their midpoint 0.07655:

| handed cut | residual | turn over? |
|---|---|---|
| fusee ratchet ×2 | 0.15208 | yes |
| set-up ratchet | 0.15208 | yes |
| alarm arbor ratchet | 0.15309 | yes |
| alarm governor saw | 0.15330 | yes |
| `alarmCam` (striking wheel) | 0.17432 | yes |
| **escape wheel club teeth** | **0.19340** | **yes** |
| `alarmLockCollar` | 0.20768 | yes |
| `alarmColSkirt` | 0.23590 | yes |

**The prose list was wrong in both directions.** It said "four saws"; there
are five saw-class cuts. It did not mention `alarmCam`, `alarmLockCollar` or
`alarmColSkirt`, which are handed and would have survived the reversal
silently — the exact failure a census exists to prevent. And it named "the
keyless sense", which measures SYMMETRIC throughout (0.00017–0.00348): the
keyless cuts are not direction-committed metal, so whatever is handed about
hand-setting is in its LAW, not its outline.

**Every one of the nine can be turned over rather than recut.** The flip tier
measures whether a body is symmetric about its own mid-plane — a 180° flip
about a diameter mirrors the cut, and costs nothing in z exactly when that
holds — and all nine read 0.0000 against a stepped-slab control at 0.04688.
So the metal is cheap to reverse. Two caveats that keep it from being the
whole answer:

- **A flip is a model trick unless the builder follows it.** Leave
  `makeEscapeWheel` authoring its tip at `c + 0.22 * pitch` with the comment
  "club tip (leading, forward)" and reverse the group's transform, and the
  constant now describes a tooth that leads backward — standing rule 1 broken
  in the quietest way available. The census says the reversal is cheap; it
  does not say to spend it on a transform. The fix is a sign in each builder,
  which `makeRatchetAndClick` already has as its `reverse` flag.
- **The mates do not inherit the verdict.** The pallet fork — the escape
  wheel's own mate, cut against it from `beatRad`/`bankRad` — measures
  0.0385–0.0679, between the poles: neither a gear nor a saw. Flipping the
  wheel says nothing about the fork, and the fork is where the escapement's
  draw lean lives.

### What would CATCH an incomplete reversal — measured by mutation, and it is 1 of 5

The two censuses say what must be reversed. This says what would notice if it
were not, which is the question that decides whether the reversal can be
landed safely at all: the danger is not the edit, it is a reversal that lands
INCOMPLETELY and looks fine, in a movement whose every collision gate is green
because nothing in the battery measures a direction.

`probe-direction-guards.mjs` answers it by MUTATION — a guard that has never
fired is not known to work, so asserting that an assert exists is a grep and
asserting that it catches is an experiment. For each commitment it copies the
tree, reverses that one sign, boots the copy, and records what complained.

| direction commitment | reversing it is… |
|---|---|
...
draw, which is used here as the must-hit control precisely because reversing
it makes the tooth torque the fork OUT of lock and the assert says so. That
also settles the pallet fork, which the outline census left "between the
poles" at 0.0385–0.0679: the fork IS direction-committed, and not in its
outline. Its stones' `f0` is rotated by `+DRAW_DEG` *"in the wheel's (+z)
rotation sense"* (`geometry.js:1194`), so the commitment lives in the stone
geometry — the same class as the winds, which is why an outline residual was
the wrong measurement for it.

**Two draft harnesses measured nothing, and both are recorded in the probe.**
A mutant that changes the SOURCE but not the METAL boots silent and reads
exactly like an unguarded commitment: the first anchor for the mainspring wind
mutated `ArchimedeanSpiral`, which `makeBarrel` uses only as its non-morph
fallback, so the shipped springs never saw it. Every mutant's built geometry is
hashed against an unmutated baseline now. And the first version of THAT rule
was too strong in the other direction — it required the metal to move, so the
train-sense row came back NO-OP *and* CAUGHT at once, because `escapeAngle` is
...
### THE LANDING — one declaration, one crossing rule, and the fork mirrored
...
- `node tools/probe-direction-guards.mjs` — **PASS**, controls hold,
  **0 of 16** direction commitments reverse silently: the 5 the item inherited,
  ...
  branch, the mainspring drum's rotation, the keyless chain's spur rate, and the
  stem one-way's cut. It also learned to SUBTRACT the baseline's own warnings:
  for one pass this tree deliberately carried one, and it would otherwise have
  ridden into every row and reported everything CAUGHT — a harness saying
  "everything is guarded" because one thing is broken. Every mutant negates a SENSE at
...
- `node tools/probe-50-clutch.mjs` — the stem clutch's tick laws reproduce the
  PRE-REVERSAL numbers exactly: take-up 0.3927 of a pitch parked and consumed,
  bank after take-up 1e-7, bank after a clean pitch 0.041666, knob drift through
  the drain 0, pick-up 0.01829, and `knobPerCrown` 0.99999. `stemClutchHandoff`
  reads seated −0.0005, backlash −0.0003, camming 0.0003, pulled 0.9857.
- Boot silent at MOVEMENT_SENSE −1 and +1, and at both alarm hands — the tree
  still builds clean with either declaration flipped back, which is what makes
  the guards a coherence check rather than a one-way ratchet.
- The battery: **33 of 36**, fingerprint 1428272738 across two virgin boots,
  local. `stemClutchHandoff` is green. The three that are red are the same
  three the third region left, and they are the WINDING ARREST — see below.

**A measurement of mine that was wrong, recorded because it nearly landed a
false claim.** A targeted `penetration` run over the same rows came back OK at
both senses (`Chain ⇄ Fusee & great wheel` 0.218/0.209 against 0.25), and I
wrote that the region had closed. The full battery says 1.491 at reserve 0.067.
The targeted run and the battery are not the same measurement — the check
carries two budget rows for that pair and the run reported the other one — so
the agreement between the senses was real and about the wrong row. A report
saying OK has not passed anything.
...
### THE FOURTH REGION — the keyless winding chain

**Reversing the going train reverses which way you turn the crown to wind.** The
keyless gearing between the crown and the fusee did not change; what changed is
which way the fusee must turn to GATHER chain. That is user-visible, and it is a
consequence rather than a preference — so it is derived here rather than
declared. `SPUR_RAD_PER_TURN` is the winding spur's world rotation per turn
banked, taken from `windLocalAt`'s own law; one mesh carries it to the crown
wheel, the crown-wheel/pinion pair to the stem, and `windSign` is read OFF the
result. Only the last relation carries a convention (the pinion turns about the
stem and the crown wheel about z), and it is stated once instead of hiding
inside a product.

- **`crownWheelSpin` was a SECOND expression of the spur's world delta**, and
  when `windLocalAt` learned the sense this copy did not: the crown wheel turned
  one way per turn banked while the spur it meshes turned the other. Nothing
  reads it — both are poses, and no gate measures whether a mesh turns the right
  way. It is derived from the spur's own delta now (§126), which is why the
  guard that used to sit there is RETIRED rather than kept green: there is no
  second copy left for a sense to be missing from, and an assert between one
  expression and itself measures nothing.
- **The input works in winding-positive knob radians.** `windStemSlip` stays in
  the knob's frame — that is what the display and the saved state mean — and
  `stemSlipLocal()` is the one conversion into the coupling's own index. The
  knob tracks the hand by construction rather than by two ratios agreeing: the
  display multiplies by `STEM_RAD_PER_TURN` and the input divides by it.
  Measured through the shipped tick, one radian of winding crown is
  **0.99999** radians of knob (the deficit is the drain during the step).

**TODO 50's saw coupling was the hard half, and it taught the item something.**
The one-way has to bear on the other flank, so both rings are cut to `windSign`.
But a mirrored pair does NOT seat where the original does: measured on the built
metal, the original's clear window is (−backlashFrac, 0] of a pitch with the
drive faces bearing at its upper edge, and the mirrored pair's is (0,
+backlashFrac) — the faces move to the OTHER end. Cut mirrored and left
unclocked, the pair measures tip-on-tip, **0.1699 buried, exactly one toothH**.
So `sawSeatOffset` clocks a mirrored pair by exactly the backlash, and the ride
law is read at the same offset.

**ONE number, in the metal and in the law — and the near-miss is why that
sentence is in the source.** Given DIFFERENT offsets, the pair still passed
every column of `stemClutchHandoff`: seated 0.0050, backlash −0.0005, camming
**−0.0256** against a 0.03 tolerance. Inside the gate, and riding a coupling
that is not there. Sharing one offset lands that column at **0.0003**. The
clocking is deliberately NOT boot-asserted, and the reason is written beside it:
`sawSeatOffset` is read by the metal and by the law, so any assert between those
two compares one call against itself — a tautology, which is exactly what the
first version of that guard was. `stemClutchHandoff` holds it instead, a battery
run away rather than a boot away.

**And the POSE TABLES were direction-committed, which is a class this item had
not met.** `STEM_CLUTCH_POSES`' "backward crown" slips and the `stemSlip` AXIS's
one-pitch backward sweep were bare negatives. On a reversed movement they drive
the pair the WRONG way past its faces: the handoff poses measured a real
tip-on-tip burial at a pose the watch never reaches, and the axis would have
swept the driving direction and left the camming reversal — the one thing it
exists to observe — unobserved, which is the §48-population gap its own comment
is about. Both carry the sense now. `probe-50-clutch.mjs` had the same defect
and the same fix: its "forward" stroke would have been an unwind.

**Two new guards, both control-tested by mutation.** The spur-rate assert
compares `SPUR_RAD_PER_TURN` against `windLocalAt`'s own law — the constant is
declared 20k lines from the law and has to be, because the clutch is built long
before the reserve constants exist, so the moment the second one is reachable it
is measured against the first. And `makeSawCoupling` now measures its own cut's
hand off the samples it laid (`userData.sawHand`), so the assert beside the
rings compares the METAL against the gearing rather than an argument against
itself.

### THE FIFTH REGION — the winding arrest, and the link that was mirrored in its own wrap

**§47's ARREST is the fifth region and it is landed.** Its finger, beak, lug and
pad are calibrated against the coil's arrival and the cone's clocking, and this
item's third region moved both. Four battery rows were red, all one mechanism,
and they came apart into three distinct defects rather than the one clocking
error the item predicted:

| row | was | now | budget |
|---|---|---|---|
| `penetration` Chain ⇄ Fusee & great wheel (float) | **1.491** at reserve 0.067 | 0.207 | 0.25 |
| `penetration` Fusee & great wheel ⇄ Winding arrest | **0.327** at wind 0.494 | 0 | 0.03 |
| `windArrestHandoff` coil ⇄ finger pad | **0.0532** at full wind | −0.0169 | ±0.03 |
| `expectedContacts` Winding arrest ⇄ Fusee & great wheel | **0** at reserve f=0.4 | 0.15 | 0.15 floor |

**1. Which side the lug comes from is the movement's.** The cone's local
rotation runs −MOVEMENT_SENSE·2π per turn banked (`windLocalAt`), so as the wind
rises the lug travels −MOVEMENT_SENSE·θ̂ and arrives from the +MOVEMENT_SENSE·θ̂
side. The beak tab's stock must run behind its working face, and the lug's
closing sweep is the SAME commitment read from the other member — the sentence
the tab's comment made ("its +θ̂ face … its stock runs −θ̂") was that arithmetic
with the sense already substituted in. Left unsigned the beak stood 0.327 inside
the lug at wind 0.494, arriving early and burying deeper all the way to full
wind, while `windArrestHandoff` still read the full-wind kiss as 0 because the
two faces are coplanar there whichever side the body is on. Flip one of the two
without the other and the pair meets stock-on-stock (measured −0.3267).

**2. The pad is a plane on a LEVER, so what a coil demands of it is a ROTATION,
not a radius.** `proudOf` reads how far the metal reaches along one ray; the
face the finger actually presents has turned 0.35 rad about its stud at full
wind and swung its own width sideways with it, so at lateral offset ℓ in the
face's own frame it stands ℓ·tanψ off where the ray says. The two agree only for
a coil whose proudest point sits ON the ray — and it is the wrap's LINK PHASE
that decides where that point sits, nothing in the solve. Reversing the movement
moved it to 0.0908 of a 0.107 half-window and the kiss opened to 0.0532.

`psiDemand` asks for the rotation instead, in closed form: rotating the pawl by
ψ carries a fixed point into the face's rest frame with leaned radial coordinate
(S−C)·r̂ + a·cosψ + b·sinψ − (zRef−z)·lean, so the face reaches it at
cos(ψ−α) = K/ρ. Three things that only measurement settled:

- It is a LEAST FIXED POINT, not one pass, because which points the face is even
  over moves with the rotation. Measured on the built metal, the face spans
  world-tangential [−0.35, +0.35] seated and [+0.09, +0.72] thrown — so an
  azimuth window centred on the pad's ray is the REST face's footprint, and
  filtering on it first hid every link the lifted face actually rides: lift fell
  to 0 across t = 0.985…0.992 and the closing-arc assert read the beak a whole
  PAD_LIFT short.
- The rest radius is BISECTED against that demand, with the stud inside the
  loop. The stud's own shove reads the rest radius too, and which link governs
  changes discretely as the face swings, so stepping by the shortfall
  limit-cycled 1.13e-3 off the designed throw and dragged the pose's
  exact-inverse assert red with it. The shortfall stays monotone in the rest
  radius whatever the discontinuities do inside it — what bisection needs and
  iteration does not.
- NO demand is EXACTLY no lift. `psiStandoff(0)` reassembles PAD_REST_R out of
  two cross products and lands a rounding step above it, and the catch-window
  assert next door tests `liftAt(...) !== 0` — 1e-16 of lift reads there as a
  finger already thrown through the lug's free pass.

**3. And the chain's link was MIRRORED INSIDE A WRAP whose path was already
right.** This is the one the item did not predict at all, and it is the same
shape as the other thirteen. `ŷ = k̂ × t̂` takes its sign from the tangent, and
the tangent reverses when the wrap does — so "+ŷ", declared INBOARD by §124,
turned outboard, and it turned twice over: the lean tips the stack toward +ŷ
AND `userData.seat` declares its crowns on that same side. Both came out
mirrored. The float row read 1.286–1.491 at EVERY reserve against its 0.25
budget (0.207 on the reference build) while the BURIAL row stayed green at 0.188
throughout, because the outer edge had simply taken the inner edge's place.

**Correcting the lean alone makes it WORSE, not better** — 2.348, which is a
correctly leaned link measured on its outer edge. That is the tell that the
frame, not the lean, is the one source. The walk's sense is now decided ONCE off
the wrap's own geometry (is ŷ pointing at the cone's axis?) and t̂ and ŷ are
negated together, which preserves the basis's handedness — and with it every
normal — while putting the link's inboard side back inboard. The plate template
is symmetric along t̂, a stadium with a rivet hole at each end, so nothing else
reads the direction of the walk. With that one decision the float row reproduces
the reference build to the last digit at all 60 reserve samples.

`§115 chain seat` is its boot guard, and it is the fourth chain guard rather
than a restatement of the other three: those are about the RUN's path, this one
about the link's own cross-section, measured as the declared crowns against the
cut's own `floorAt` at full wind, held at the float row's own 0.25 so the two
carry ONE number. Mutated, it reads 1.489 and says so
(`probe-direction-guards.mjs`, now 17 commitments, 0 silent).

**4. And the analytic twin had its own copy of that frame, which is how one
defect cost two landings.** `linkOuterPtsNear` — the sampler the arrest's reach
tables and the finger's whole fold are solved against — replicated
`buildChainLinkGeometry`'s frame law verbatim, comment and all, and so kept
modelling the link the mirrored way after the metal was fixed. The arrest then
solved its pad arm against a chain that no longer existed: `expectedContacts`
`Winding arrest ⇄ Chain` read 0.1325 at reserve f=0.1 against its 0.15 floor,
with the arm's bottom corner 0.089 inside a link 0.097 below the hub's band —
0.025 inside the sphere-of-margin demand the reach table would have made had it
been looking at the right metal. The frame law is now ONE function
(`chainLinkFrame`, with `chainWalkFlip` beside it) above both readers, and the
grep that proves it is that the pin-axis expression appears once in the file.
With the table reading the built chain again the fold re-solves itself: the pad
arm's station goes out, PAD_T grows with it (1.0293 → 1.0796 — it is the gap the
pad bridges, not a literal), and the row measures 0.1703.

This is the item's own headline finding for the fourth time in one region, so it
is worth stating flatly: the recurring defect is not a wrong sign, it is a
SECOND COPY. Three of the arrest's four defects were a direction held in two
places with one of them carrying the sense, and the fourth was a whole law held
in two places.
...
moved is measured numbers under a fold that re-solved, which is what a re-solve
should look like. One row left the `inspection` report and is worth naming
rather than passing over: `Fusee & great wheel ⇄ Winding arrest` is no longer
listed as CONTACTING, and it is because the beak now KISSES its lug instead of
biting it. Measured at full wind, `meshClearance(windArrestBeak,
windArrestLug)` = **6.0e-9** — 0 at tension 0.9999, 0.032 by 0.999 — so the pair
touches, the sweep's contact classifier does not count 6e-9, and
`windArrestHandoff` (the gate that owns the claim) reads it as 0 at both
parities. A pair that stopped overlapping is the direction this row was supposed
to move.

### THE LAST ROW — a measurement problem, measured

`penetration`'s `Keyless works ⇄ Winding clutch` read **0.17 at stemSlip
0.421** — exactly one `toothH`, which is what an MTV reports for two INTERLEAVED
combs the moment they touch at all, whatever the depth. Clearing an interleave
means lifting one ring clean out of the other's valleys, and no smaller push
exists in any direction, so the row could only ever have returned ~toothH or
zero: it was never measuring its own claim. The proof of that is the REFERENCE
build at MOVEMENT_SENSE +1, which read GREEN on the same geometry while sitting
0.0045 into the identical hairline — green because its axis happened to sample
poses where the rings never touched at all. Reversing the movement moved the
sampling, not the fit.

This item filed it as wanting "either a depth measure that understands
interleave or a declared contact". It got the first. **The pair's own free
coordinate is the SLIP ANGLE**, so `sawRideDepth` bisects that instead: the
smallest rotation about the shared axis, either way, that clears the boolean
intersection — `mtvDepth`'s minimum-translation search one coordinate over,
...
lives in `probe-50-clutch.mjs` as its own tier: pre-rotate the clutch by 0.02
and 0.05 rad and the reading moves by exactly 0.02 and 0.05; rotate it the
other way and the pair clears outright. Two failed measures preceded it and are
worth recording, because both came back CLEAN while measuring nothing — the
catalogue in `.claude/skills/instruments/SKILL.md` names both classes. An axial
surface-vs-surface probe read 0 for the shipped build AND 0 for a 0.12 push,
because a ray along the axis meets each ring's engaging face from BEHIND and a
FrontSide material does not report a backface: `clutchSaw` returned 0 hits at
every radius while `windPinionSaw` returned 2. And before that, the sampling
annulus was taken from a bounding box, whose corner is not a ring's outer
radius and whose centre is not its bore.

**What else this landing does NOT close**, named rather than left as omissions:
the fusee groove still has no instrument (its hand is read from the generator's
emitted crest points, not from the welded metal), and the stem coupling's
CLOCKING is held by `stemClutchHandoff` in the battery rather than by a boot
assert, for the tautology reason above.

**The shape of every finding, in one line.** Fourteen of the seventeen defects
this item turned up were the same thing: ONE direction written down TWICE, in
two files or two laws, with only one copy carrying the sense — the groove and
the wrap, the tangent branch and the hook, a saw and its analytic twin, the
reserve term and the train, the spur and the crown wheel, the ring's cut and the
ride law's index, the beak's stock and the lug's sweep, the link's lean and the
crowns that declare where it seats. The fix that keeps working is not a sign, it is ONE SOURCE; the
guard that keeps working measures a physical invariant both copies must satisfy
(the chain lies in its groove, two pulleys on one belt turn together, a click
climbs the ramp, a saw's cut hand matches the direction that drives it), never
restates either copy.
...
## 117. The alarm setting chain does not transmit at its tooth ratios — one sign FIXED, a three-law contradiction OPEN

Found by item 116's phase instrument: the alarm setting wheel ⇄ idler 1 mesh
read **37.44% of a pitch off anti-phase** at τ = 0, on a chain
`solveGearChain` solves and boot passes in silence. It is not a phase defect.
`tools/probe-mesh-transmission.mjs` asks the prior question — **does each
declared mesh actually turn its neighbour by the amount the metal says?** —
and the answer is no, in a way a single sweep could never show.

**Each input has to be walked separately, and that is the whole finding.**
The chain takes two: the crown at one end, and the hour's back-drive through
the disc's friction seat at the other. Measured as driven-spin / driver-spin
against the pitch-radius bar `−r_A/r_B`:

| declared mesh | under the CROWN | under the HOUR |
|---|---|---|
| disc rim → i1b (keyed to i1) | **+1.071** (want −1.071) | −1.071 ok |
| setting wheel → i1 | −1.071 ok | **driver STILL** while i1 turned −3.366 |
| i1 → i2 | −0.757 ok | −0.757 ok |
| i2 → arbor pinion | −3.700 ok | **+3.700** (want −3.700) → **FIXED** |

Every one of those rows is exact to six figures in the direction it works, so
nothing here is a tolerance: each law is simply written for one input and
silent or inverted for the other. Only the two idlers are correct under both.

**Fixed: the arbor pinion's back-drive sign.** `alarmRotor.rotation.z` read
`-alarmSetRot - 3*_bd`; the crown term was right (−3.700 measured) and the
hour term inverted. Corrected to `+3*_bd` and re-measured at −3.700 under
both inputs. `ALARM_BD_SIGN` is deliberately NOT the fix: i1 and i2 read the
same constant and are correct. Nothing reads the rotor's angle back — it is
posed, never sensed — so the correction moves the arbor rod and its bevel
mount and nothing else.

**Open, and NOT a sign: three laws that cannot all hold.** The remaining two
rows are one defect seen twice, and it is a design contradiction:

1. **The gearing** makes the setting wheel and the disc ONE 1:1 pair. Both
   are 30 T at the dial centre; the disc meshes `i1b`, `i1b` is keyed to `i1`
   (`alarmSetI1Spin.add(i1b)`, `src/main.js:13374`), and `i1` meshes the
   setting wheel. 30 → 28 → 30 co-rotates, and both centre distances measure
   the pitch sum exactly. So the wheel must carry whatever the disc carries.
2. **The disc carries the hour**, and its law is PINNED by the trip. Measured
   at three alarm settings (0, 0.7 and 1.9 crown turns): the feeler's pin
   bottoms at full `ALARM_PIN_DROP` when the hour hand's world azimuth equals
   the alarm hand's, to an offset of 0.02185 rad that is IDENTICAL at all
   three — a constant, i.e. the sampling plateau, not an error. Flipping
   `ALARM_DISC_SIGN` would make the alarm ring at the wrong time, so the
   disc's law is not available as the thing to move.
3. **The armed coupling requires the wheel to equal the TUBE.** The pin-B
   face cam reads `relB = wrapPi(alarmTubeShownA − wheelAngle)` and
   "armed-seated: relB 0 ⇒ minimum height ⇒ the coupling"
   (`src/main.js:34098-34104`). The tube holds the set time — measured, it
   does not move under the hour, which is what an alarm hand must do.

(1) and (2) say the wheel follows the hour. (3) says it must not, or the
coupling un-seats once every 12 h. The tick law currently obeys (3) and
...
### The travel is MEASURED — the topology is feasible, and it constrains the reader

`tools/probe-feeler-travel.mjs`, over the 42-pose net (every declared axis at
f ∈ {0, 0.5, 1}), 72 stations of 5°, both controls passing. **The band is not
full; the ARM is long.** Keeping only the feeler's points inside each radius
and orbiting that:

| a reader reaching | stations as good as shipped | best gap |
|---|---|---|
| **r 3.6** | **72 of 72 — the whole 360°** | **0.4153** |
| r 5 | 18 of 72 — 90° | 0.1509 |
| r 7 | 13 of 72 — 65° | 0.0510 |
| r 10 | 13 of 72 — 65° | 0.0510 |
| r 14.261 (today's whole arm) | 9 of 72 — 45°, in 5–15° fragments | 0.0510 |
...
where it stands, with a green battery — so the movement contains real
sub-margin proximities it accepts, and a sweep demanding the margin everywhere
asserts something untrue of the tree. (It also demonstrates TODO 6's blind
spot from a new direction: nothing gates that 0.0458.)

**What this constrains.** The reader must be a SHORT member at the track
radius, not a long lever. Which makes the output path the real design problem
rather than the travel: today's arm reaches r 14.261 to meet a release lifter
at r 16.942, and that run's length spans **2.682 → 31.203 across the orbit, a
spread of 28.521** — an order of magnitude, so a simple link cannot do it. The
output has to be taken on the reader's own axis, or through a carrier that
orbits with it. Price that before building anything.

**Residue.** The measurement is of the CURRENT feeler's envelope swept through
stations; it says a short reader has room, not that any particular short
...
stack; the slack is now stated rather than mis-derived). *(Overtaken one
landing later: the owner directed the respend, and item 119's measurement
...
it is the finding. §188's freed-headroom bound (offset ≥ 2.402) and item
118's correction of it (≥ 2.002) both held the hour hand's metal off the
alarm hand's bored collet **in z alone**. Measured per-mesh with radii
(the item-118 method carried one step further): the collet is a ring at
r 2.67..3.30, and the hour hand carries NO metal between its boss rim
(r 1.26) and its blade root (r 5.52) — the two planes those records held
apart can never meet. The same scan over EVERY fixed mesh behind the
descending stack (radial overlap required before a z-gap counts) found
the real governing pair: the **alarm blade's top against the hour
blade's keel**, coplanar annuli sharing 15 u of radius, gap 0.9722 at
the shipped 2.6. Next binder after it: the raised dial markers under the
hour tip, 0.6 u further and grazing 0.0007 u of shared radius.

**CLOSED in the same landing.** The offset derives:
`(ALARM_HAND_Z − DIAL_T) + topRise_alarm + floorDrop_hour + CLEAR_MARGIN
= 1.1 + 0.1759 + 0.3519 + 0.15 = 1.7778`, stored 1.778 (rounded up at
3 dp) — the stack comes down 0.822 u = 0.31 mm and the front chain
(`handFront` → crystal → bezel → band front → §39 cased depth) follows by
measurement, zero edits. The §125 lane assert is rewritten in two clauses
with the radial truth in it: blade↔blade two-sided (thin warns AND float
warns — item 118's maximum-air lesson at build time), hub↔blade guarded
by the measured radial gap (4.0 u today; if a change ever brings the
alarm blade's root inboard of the hub rim, the old z comparison re-arms
verbatim). `probe-hand-stack`'s alarm↔hour lane measure gains the same
radial awareness — the hub now legally interleaves the collet's z-band,
which the old whole-subtree measure would misread as contact — and an
acceptance clause holding the lane bound at CLEAR_MARGIN in both
directions.

**Residue, named:** the scan that sized this covers the build pose plus
the 43-pose net's z-behaviour for these members (their z is
pose-independent; rotation is about the common axis). Transients between
...
are absences, not overlaps:

- **The hour hand's boss is SOLID** (mesh rMin = 0) with outer r 1.2555 —
  but the hour TUBE it is "mounted on" spans r 2.05..2.50. The collet is
  radially INSIDE the bore of its own arbor: it grips nothing, touches
  nothing, and could not be assembled onto the tube at all. The hand
  turns because it is a child of `hourWheelGroup`, not because any
  surface drives it.
- **The minute hand's boss is likewise solid** (rMin = 0). It caps the
  stack over the cannon pinion's nose rather than being bored and
  friction-fitted to it, and the hand rides `handsGroup`, not the cannon
  pinion.
- **Both central blades detach from their own bosses radially**: the hour
  blade's root starts at r 5.52 against a boss rim of 1.26 (4.3 u of
  void), the minute blade's at r 7.86 against 1.25. Each hand is two
  disconnected bodies posing as one — `assembly`'s connectivity would
  call this a split, but the Dial/Hour wheel units are outside
  `ASSEMBLY_SCOPE`, so the rows are §107's reported residue today.

**Fix path:** bore each central boss to its arbor (hour boss bore =
`HOUR_TUBE_OUTER` + running fit, seated ON the tube's outside like the
alarm hand's collet already is; minute boss bore = the cannon pinion's
nose + friction fit), bridge each blade root to its boss with real metal,
and declare the two display joints in `INTRA_UNIT_CONTACTS` /
